...
def self_play_worker(worker_id, input_queue, pipe_conn, experience_queue, num_games=10000):
    ...
    env = ReversiEnv()
    mcts = MCTS(num_simulations=100) # Lowered to 100 for faster generation
...
if __name__ == "__main__":
    ...
    if load_checkpoint:
        checkpoint_path = "checkpoints/reversi_model_game_5500.pth"
        master_model.load_state_dict(torch.load(checkpoint_path, map_location=device, weights_only=True))
        print("Master Model loaded from checkpoint and ready for training!")
        

    optimizer = optim.Adam(master_model.parameters(), lr=0.001, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=1000, gamma=0.9)
    
    replay_buffer = ReplayBuffer(capacity=500000)

    games_played = int(checkpoint_path.split("_")[-1].split(".")[0]) if load_checkpoint else 0
        ...
        while True:
            try:
                ...
                if games_played % 20 == 0 and len(replay_buffer.buffer) > 10000:
                    v_loss, p_loss = train_network(master_model, optimizer, replay_buffer, batch_size=256, device=device)
                    ...
                    if games_played % 100 == 0:
                        torch.save(master_model.state_dict(), f"checkpoints/reversi_model_game_{games_played}.pth")
